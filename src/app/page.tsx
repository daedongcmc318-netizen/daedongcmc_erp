...
export default async function DashboardPage() {
  ...
  return (
    <div className="px-8 py-7 max-w-[1500px] mx-auto">
      ...
      {/* 1주 이내 보고 마감 */}
      <UpcomingReports items={data.upcomingReports} todayMs={data.todayMs} />
...
function UpcomingReports({ items, todayMs }: { items: UpcomingItem[]; todayMs: number }) {
  return (
    <div className="bg-white rounded-2xl border border-slate-200/70 shadow-card p-5 mb-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <table className="w-full text-xs">
            <thead className="bg-slate-50">
              <tr className="text-[10.5px] text-slate-500 font-medium">
                ...
                <th className="text-left px-3 py-2 w-20">D-day</th>
              </tr>
            </thead>
            <tbody>
              {items.map((it, idx) => {
